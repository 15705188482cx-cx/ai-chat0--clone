...
import { parseHtmlFormat } from "./strategies/wechatHtmlParser";
import { parseCsvFormat } from "./strategies/csvParser";
...
export function parseChatFile(
  ...
  let rawMessages;
  if (fileType === "json") {
    rawMessages = parseJsonFormat(content);
  } else if (fileType === "html") {
    rawMessages = parseHtmlFormat(content);
  } else if (fileType === "csv") {
    rawMessages = parseCsvFormat(content);
  } else {
    rawMessages = parseTxtFormat(content);
  }
